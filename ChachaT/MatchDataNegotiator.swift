...
class MatchDataNegotiator: NSObject {
    var delegate: MatchDataNegotiatorDelegate?
    ...
    init(delegate: MatchDataNegotiatorDelegate) {
        super.init()
...
protocol MatchDataNegotiatorDelegate {
    func passMatches(_ matches: [Connection])
    func passChatRooms(_ rooms: [ChatRoom])
}

extension MatchesViewController: MatchDataNegotiatorDelegate {
    func passMatches(_ matches: [Connection]) {
