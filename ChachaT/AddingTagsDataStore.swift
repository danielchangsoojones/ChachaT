...
class AddingTagsDataStore: SuperTagDataStore {
    ...
    init(delegate: AddingTagsDataStoreDelegate) {
        ...
        loadCurrentUserTags()
        convertTagsToParseTag()
    }
    
    //Time Started: 10:03 AM
    func convertTagsToParseTag() {
        let query = Tags.query()! as! PFQuery<Tags>
        query.includeKey("createdBy")
        query.whereKeyExists("createdBy")
        query.findObjectsInBackground { (tags, error) in
            if let tags = tags {
                for tag in tags {
                    self.convertATag(tag: tag)
                }
    ...
    fileprivate func convertATag(tag: Tags) {
        let specialtyNumbers = [tag.hairColor, tag.gender, tag.sexuality, tag.politicalGroup, tag.ethnicity]
        for num in specialtyNumbers {
            if let specialtyTitle = SpecialtyTagTitles(rawValue: num) {
                if num < 0 {
                    //it is a private/none tag, but we are just going to make all negative tags private
                    let specialtyCategory: String = specialtyTitle.associatedSpecialtyCategoryTitle!.rawValue
                    savePrivacyTag(specialtyCategory: specialtyCategory, user: tag.createdBy)
                } else {
                    saveSpecialtyTag(title: specialtyTitle.toString, specialtyCategory: specialtyTitle.associatedSpecialtyCategoryTitle!.rawValue, user: tag.createdBy)
                }
            }
        }
        
        if !tag.genericTags.isEmpty {
            convertGenericTags(titles: tag.genericTags, user: tag.createdBy)
        }
    }
    
    
    func saveSpecialtyTag(title: String, specialtyCategory: String, user: User) {
        removeSpecialtyTag(specialtyCategory: specialtyCategory, user: user)
...
//                let jointParseTagToUser = self.createJointParseTagToUser(parseTag: parseTag, user: user)
//                jointParseTagToUser.saveInBackground()
            } else if let error = error {
                print(error)
            }
        }
    ...
    fileprivate func removeSpecialtyTag(specialtyCategory: String, user: User) {
        for parseTag in currentUserParseTags where parseTag.dropDownCategory?.name == specialtyCategory {
            ...
            deleteJointParseTagToUser(tagTitle: parseTag.tagTitle, user: user)
        }
    }
    
    fileprivate func deleteJointParseTagToUser(tagTitle: String, user: User) {
    ...
    func savePrivacyTag(specialtyCategory: String, user: User) {
    ...
    func convertGenericTags(titles: [String], user: User) {
        for title in titles {
            saveNewTag(title: title, user: user)
        }
    }
    
    func saveNewTag(title: String, user: User) {
        let query = ParseTag.query()!
        query.whereKey("title", equalTo: title)
